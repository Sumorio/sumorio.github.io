...
$.ajax({
  ...
  data: {
    key: "47YBZ-YYW3B-ZK4UY-JJWTK-CJAQS-UDFF7",
    output: "jsonp",
  },
  dataType: "jsonp",
...
function showWelcome() {  
    ...
    let dist = getDistance(113.34499552, 23.15537143, 108, 30); //这里换成自己的经纬度  
    ...
    let timeChange;  
    let date = new Date();  
    if (date.getHours() >= 5 && date.getHours() < 11) timeChange = "<span>早上好！</span>";  
    ...
    else if (date.getHours() >= 16 && date.getHours() < 19) timeChange = "<span>黄昏啦！</span>";  
    else if (date.getHours() >= 19 && date.getHours() < 24) timeChange = "<span>晚上好！</span>";  
    else timeChange = "夜深了，早点休息，少熬夜。";  
  
    try {  
        //自定义文本和需要放的位置  
        document.getElementById("welcome-loc").innerHTML =  
